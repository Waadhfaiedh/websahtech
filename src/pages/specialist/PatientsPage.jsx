...
export default function PatientsPage() {
  ...
  const fetchPatients = async () => {
    try {
      ...
      const count = res.data.count ?? (Array.isArray(patientsData) ? patientsData.length : 0);
  ...
  const filtered = patients.filter(p =>
    ...
    p.patient?.medicalHistory?.[0]?.title?.toLowerCase().includes(search.toLowerCase())
  );
      ...
      <div className="p-8 animate-fadeIn">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(patient => (
            <Link to={`/specialist/patients/${patient.userId}`} key={patient.userId}
              ...
              <div className="flex items-start gap-3 mb-4">
                <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center flex-shrink-0">
                  <span className="text-primary font-bold">{patient.fullName?.charAt(0) ?? '?'}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-gray-900 group-hover:text-primary transition-colors truncate">
                    {patient.fullName}
                  </h3>
                  <p className="text-sm text-gray-500">{getPatientAge(patient)} ans</p>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-800 font-medium">{getPrimaryCondition(patient)}</span>
                </div>
                <div className="flex items-center gap-2">
